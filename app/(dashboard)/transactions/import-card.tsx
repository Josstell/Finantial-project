...
export const ImportCard = ({ data, onCancel, onSubmit }: Props) => {
  const [selectedColums, setSelectedColums] = useState<SelectColumsState>({})
  ...
  const handleContinue = () => {
    ...
    const formattedDate = arrayOfData.map((item) => ({
      ...item,
      amount: convertAmountToMiliunits(parseFloat(item.amount.slice(1))),
      date: format(parse(item.date, dateFormat, new Date()), outputFormat),
    }))
    console.log({ arrayOfData, formattedDate })
